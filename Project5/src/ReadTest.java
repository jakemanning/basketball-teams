...
public class ReadTest {
	...
	public int testPersonFile() {
		...
		try {
			testPersonMap = db.readPersonsFromFile("players.csv");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	@Test
	public void testTeamFile() {
		DataBaseModel db = new DataBaseModel();

		try {
			db.readPersonsFromFile("players.csv");
		...
		// System.out.println(testIdentificationMap.get("SEA").getSeasonMap().get(2006).getRosterMap().get("RASHARD
		// QUOVON LEWIS").getCityOfBirth().getLatitude());
