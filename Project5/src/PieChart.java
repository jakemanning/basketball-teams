...
public class PieChart extends JFrame
{
	 ...
	 * <P>
	 * @param rosterPassed Team roster of Persons passed from the User.
	 * @param stringPassed Particular instance of the Pie Chart.
	 */
	...
	{	
		...
		Color[] color = {Color.red,Color.blue,Color.green,Color.orange,Color.cyan,Color.lightGray,Color.magenta,Color.white,Color.pink,Color.yellow};
		
		//Sets acceptable ratio to quality of the window and the time to construct it.
		...
		/* JTextArea object which holds the String result of the sort by first name method.*/
		JTextPane jp = new JTextPane();
		
		jp.setBackground(Color.BLACK);
		//If there are not enough colors to represent Persons in the list, more are made.
		for(int keyIndex = 0, colorIndex = 0, multipleOfSize = 0, total = 0; keyIndex < agePercentMap.size(); ++keyIndex)
		{
			...
			if(keyIndex >= color.length  && (keyIndex % color.length) == 0)
			{
				...
				{
					Color newColor = new Color((int)(Math.round(color[index].getRed()*.96)),(int)(Math.round(color[index].getGreen()*.96)),(int)((Math.round(color[index].getBlue()*.96))));
			...
			//Fills in the Pie Chart according the index of the percent of ages and the order of the formation of ages.
			if(keyIndex == 0)
			{
				g2.fill(new Arc2D.Double(110, 80, 300, 300, 0, agePercentMap.get(ageList.get(keyIndex))*3.6, Arc2D.PIE));
			...
			{
				g2.fill(new Arc2D.Double(110, 80, 300, 300, total, agePercentMap.get(ageList.get(keyIndex))*3.6, Arc2D.PIE));
				total += agePercentMap.get(ageList.get(keyIndex))*3.6;
		...
		/* JScrollPane object which holds the textOfResult object making a scroll bar for the JOptionPane*/
	 ...
	 * @param tp	The text Pane to be put next to the Pie Chart.
	 * @param msg	The element displaying the particular age.
	 * @param c		The color set to the element.
	 */
	...
	/**
	*Creates a Hash Map for the percentages of age in the passed list of Persons
	*<P> 
	* @return LinkedHashMap with the key of ages and the value of percentages.
	*/
	public LinkedHashMap<Integer,Integer> createHashMap()
	{		
		//Constructs a new ArrayList for the Integers that will be grouped for their makeup of a roster.
		...
		for(Integer i:ageList)	
		{
			...
			//If the percent was not already calculated, it is placed in the LinkedHashMap.
	 ...
	 * @param ageList ArrayList of ages.
	 * @param age	  The particular in which to calculate the percent in the ArrayList of ages.
