 ...
 * March 27, 2015
 * <P>
 * Contains data on people with a name, date of birth, city and state of birth, age at the present
 * time; or when the person died, and the date of the person's death, if included.
 * </P>
 * @version 1.1
 */
...
public class Person implements Comparable<Person>, Serializable
{
	...
	/**Stores the age of each person from date of birth to present or date of death, as an int*/
	private int age;
	...
	/**
	 * A Constructor which allows a person to be initialized with any desired name, date of birth,
	 * city and state of birth, and age at the present time.
	 * <P>
	 * @param name :the name of the person (String)
	 * @param dateOfBirth :the date of the person (String)
	 * @param locationOfBirth :the city and state of the person (String)
	 * @throws ArrayIndexOutOfBoundsException to check if the date of birth or date of death is read in correctly
	 */
	public Person(String name, String dateOfBirth, String locationOfBirth,boolean shouldEdit) throws ArrayIndexOutOfBoundsException,IllegalFieldValueException,NumberFormatException
	{
		...
		this.stateOfBirth = MapClass.putInMapListIfAbsent(DataBaseModel.getJListOfStates(),this.stateOfBirth.getStateName().toUpperCase(),this.stateOfBirth);
		...
		this.cityOfBirth = MapClass.putInMapListIfAbsent(DataBaseModel.getJListOfCities(),this.cityOfBirth.getCityName().toUpperCase() + this.stateOfBirth.getStateName().toUpperCase(),this.cityOfBirth);
	...
	/**
	 * A Constructor which allows a person to be initialized with any desired name, date of birth,
	 * city and state of birth, age when the person died, and date when the person died.
	 * <P>
	 * @param name :the name of the person (String)
	 * @param dateOfBirth :the date of the person (String)
	 * @param locationOfBirth :the city and state of the person(String)
	 * @param dateOfDeath :the date of death of the person (String)
	 * @throws ArrayIndexOutOfBoundsException :to check if the date of birth or date of death is read in correctly (Exception)
	 */
	public Person(String name, String dateOfBirth, String locationOfBirth, String dateOfDeath,boolean shouldEdit) throws ArrayIndexOutOfBoundsException,IllegalFieldValueException,NumberFormatException
	{
	 ...
	 * @param dateOfBirth :the date of the person (String)
	 * @param locationOfBirth :the city and state of the person (String)
	 * @param stateOfBirth the state of the person
	 * @param shouldEdit to test whether the person should be edited or not
	 * @throws ArrayIndexOutOfBoundsException to check if the date of birth or date of death is read in correctly
	 */
	public Person(String name, String dateOfBirth, State stateOfBirth,City cityOfBirth,boolean shouldEdit) throws ArrayIndexOutOfBoundsException,IllegalFieldValueException,NumberFormatException
	{
		this.shouldEdit = shouldEdit;
		//Split aspects of the Person to the respective variables.
		String totalName[] = name.split(" ");
	 ...
	 * @param dateOfBirth :the date of the person
	 * @param stateOfBirth :the state of the person
	 * @param cityOfBirth the city of the person
	 * @param dateOfDeath :the date of death of the person
	 * @throws ArrayIndexOutOfBoundsException :to check if the date of birth or date of death is read in correctly
	 */
	public Person(String name, String dateOfBirth, State stateOfBirth,City cityOfBirth, String dateOfDeath,boolean shouldEdit) throws ArrayIndexOutOfBoundsException,IllegalFieldValueException,NumberFormatException
	{
		this.shouldEdit = shouldEdit;
		//Split aspects of the Person to the respective variables.
		String totalName[] = name.split(" ");
	 ...
	 * <P>
	 * Algorithm:<br>
	 * 1. Uses the static method in the String Class to break apart the date of birth and date of death if present. <br>
	 * 2. Uses try and catch to check if an ArrayIndexOutOfBoundsException is reached.<br>
	 * 3. Uses the inherent classes in the Joda Framework to calculate the difference in years between a period of time.<br>
	 * <P>
	 * @return number of years between date of birth and the present, or date of death if dead. 0 if the person has no readable date of birth or date of Death (int)
	 * @throws ArrayIndexOutOfBoundsException to check if the date of birth or date of death is read in correctly (Exception)
	 */
	public int calculateAge() throws ArrayIndexOutOfBoundsException,IllegalFieldValueException,NumberFormatException
	{
		...
		else
		{
			if(!this.dateOfBirth.equals("00/00/0000"))
			{
				...
				LocalDate deathDate = new LocalDate (year, month, day); //Death date
		...
		return 0;//returns default value which tells the toString method to "Skip"
	...
	public ArrayList<Person> getNeighbors()
	{
		...
		for(Season season:seasonList.values())
		{
			...
			{
				if(!personKey.equals(this.plainText.toUpperCase()))
				{
	...
	public ArrayList<ArrayList<Person>> getShortestPath(Person target,int degrees)
	{
		...
		{
			listOfNeighborPaths.addAll(neighbor.getShortestPath(neighbor,degrees + 1));
		}

		ArrayList<Person> shortestPath = listOfNeighborPaths.get(0);
		...
		shortestPath.add(0,this);
		if(!shortestPath.contains(target))
		{
	...
	/**
	 * Overrides the toString() method in the object Superclass to print out all of the Life History Data
	 * <P>
	 * @return a String containing the life History Data in a Person (String)
	 */
	public String toString()
	{
		String stringToReturn = this.fullName + "," + this.dateOfBirth + "," + this.cityOfBirth.getCityName() + "," + this.stateOfBirth;

		if(this.dateOfDeath == null && this.age > 0)//if the person hasn't died yet, and has a positive age.
		{
		...
		return "WARNING, DATA IS INCORRECT OR NON-EXISTENT.";//if the person's age is negative
