...
* March 27, 2015
* <P>
* Reads in a list of persons from a file into an ArrayList, and creates an object for each 
* Person, allowing the data to be accessed and manipulated
* <P>
...
//	private String cityFile;
	...
	/**Instantiates a LinkedHashMap used to store all the Teams in a given data file, with the keys as Team names, and values as Teams.*/
...
//	 * <P>
//	 * @param personName :The exact name of the person to be checked against (String)
//	 * @return whereItIs :The index of where the person to check is in the ArrayList (int)
//	 */
...
////		
////		
////		//WhereItIs, the index where the key, nameOnlyPerson A person with only a name is compared to the rest of the list
//		int whereItIs = -1;
////		
////		//Checks the list of Persons for someone who the exact name as the input and stores its index.
...
//		Person nameOnlyPerson = new Person(personName,"1/1/1970","Dallas, TX",false);//a person just for the purpose of finding a persons's data based on name
//		
//		//WhereItIs, the index where the key, nameOnlyPerson A person with only a name is compared to the rest of the list
//		int whereItIs = Collections.binarySearch(listOfPersons,nameOnlyPerson);//whereItIs, the index where the key, nameOnlyPerson A person with only a name is compared to the rest of the list
//		
...
//	 * <P>
//	 * @param partialPersonName :The partial name of the person to be checked against. (String)
//	 * @return partialPersonList :A list of all matches of the partial name. (ArrayList)
//	 */
...
//		
//		//Checks the list of Persons that contain the String entered by the User, and returns a list of matches.
...
//		{
//			System.out.println("Sorry, " + partialPersonName + " isn't in the file: " + peopleFile);
...
//	 * @return cityToCreate  City that needs to be added to the list of States. (City)
//	 */
...
//
//
//			if(city != null && city.getCityName().equals(cityToCheck) && city.getStateName().equals(stateOfBirth.getStateName()))
//			{
//				cityToCreate = city;
//			}	
//
...
//	}
	...
	* <P>
	* Algorithm:;
	* 1. Uses a FileReader and wraps it in a BufferedReader, and uses the buffered reader to read each line of comma separated values<br>
	* 2. Then uses the static method in the String class, split, to break apart the comma separated values. <br>
	* 3. Then reads the individual cities into an ArrayList of type String, containing the latitude and longitude of each.<br>
	* <P>
	* @param cityFile the file from which to get input from
	* @exception IOException Signals that an I/O exception of some sort has occurred (Exception)
	...
	*/
	public Map<String,City> readCitiesFromFile(String cityFile) throws IOException
	{
		...
		while(city != null)
		{
			...
			stateList.get(testState.getStateName().toUpperCase()).sortListOfCities();
	 ...
	 * 1. Uses a FileReader and wraps it in a BufferedReader, and uses the buffered reader to read each line of comma separated values<br>
	 * 2. Then uses the static method in the String class, split, to break apart the comma separated values. <br>
	 * 3. Then reads the individual Persons into an ArrayList of type people, based on whether the line has a date of death or not.<br>
	 * <P>
	 * @exception IOException Signals that an I/O exception of some sort has occurred (Exception)
	 * @return listOfPersons :the ArrayList that is read in, only used for testing purposes (ArrayList)
	 */
	public Map<String,Person> readDataFromFile(String peopleFile) throws IOException
	{
			...
			if(array.length < 5)
			{
				if(!locationOfBirth.contains("?") && !locationOfBirth.contains("0") && !array[2].trim().isEmpty())
				{
					try
					{
						...
						personCheck = MapClass.putInMapListIfAbsent(personList,personCheck.getTextName().toUpperCase(),personCheck);
					}
	 ...
	 * 1. Uses a FileReader and wraps it in a BufferedReader, and uses the buffered reader to read each line of comma separated values<br>
	 * 2. Then uses the static method in the String class, split, to break apart the comma separated values. <br>
	 * 3. Then reads the individual Team into an ArrayList of type String, each index containing the Team name, city, state, and roster..<br>
	 * <P>
	 * @return teamHashMap a LinkedHashMap with the keys as Team names, and values as Teams.
	 * @throws IOException signals that an I/O exception has occurred. (Exception)
	 * @param teamFile the file to read teams from
	 */
	public Map<String,Identification> readTeamsFromFile(String teamFile) throws IOException
	{
		...
		while(lineInfo != null)
		{
			if(lineInfo.contains(":"))
			{
				...
				createdIdentification = MapClass.putInMapListIfAbsent(identificationList,teamIdentification.toUpperCase(),createdIdentification);
			}
			else if(lineInfo.contains(";"))
			{
				...
				teamCity = MapClass.putInMapListIfAbsent(cityList, teamCity.getCityName().toUpperCase() + teamState.getStateName().toUpperCase(), teamCity);
				...
				for(int index = 4; index < seasonInformation.length;++index)
				{
					...
					for(String identification:identificationList.keySet())
					{
						if(!identification.equals(createdIdentification.getIdentification()))
						{
							...
							{
								if(year.equals(teamYear) && seasonList.get(year).getRosterMap().containsKey(personName))
								{
									inAnotherTeam = true;
								}
							}
...
//	 * @return cities :An ArrayList of all cities in a particular state. (ArrayList)
//	 */
...
//	 * @return cities :An ArrayList of all cities in a particular state. (ArrayList)
//	 */
//	public ArrayList<Person> searchForCitiesInStateObject(String stateToSearch)
//	{
//		//Instantiation of ArrayList to hold a list of Persons and a boolean to check if the correct state was found.
...
//	 * <P>
//	 * @param stateToSearch State input by the User in which to search through for the city. (String)
//	 * @param cityToSearch	City input by the User in which to display the list of Persons within the city. (String)
//	 * @return word			Displayed attributes of Persons within a particular City. (String)
//	 */
//	public String searchForCityInState(String stateToSearch,String cityToSearch)
//	{
//		Collections.sort(listOfJListCities);
//		
//		//Construction of String to hold the Person in a city and boolean to show that one was found.
//		String word = "";
//		boolean isIn = false;
//		
...
//	 * @return word			Displayed attributes of Persons within a particular City. (String)
//	 */
//	public ArrayList<Person> searchForCityInStateObject(String stateToSearch,String cityToSearch)
//	{
//		Collections.sort(listOfJListCities);
//		
//		//Construction of String to hold the Person in a city and boolean to show that one was found.
...
//	 * <P>
//	 * @param personName :The name of the person to be printed that we are searching for. (String)
...
//		}
//		
//		return listOfPersons.get(whereItIs).toString();// listOfPersons.get(whereItIs).toString();//otherwise, return the person at the index in the ArrayList listOfPersons.
...
//		}
//		
//		return listOfPersons.get(whereItIs);// listOfPersons.get(whereItIs);//otherwise, return the person at the index in the ArrayList listOfPersons.
//	}//end of searchForLifeData method.
...
//	 */
//	public String searchForPeopleInCity(String cityToSearch,String stateToSearch)
//	{
//		//Sort by the Last Name Comparator
//		Collections.sort(listOfPersons, new LastNameComparator());
//		
//		//Variable of the specific city to search and ArrayList for its respective Latitude and Longitude.
//		String word = cityToSearch + ":\n";
//		ArrayList<String> stringList = new ArrayList<String>();
//		
//		//Searches the list of Person's for the specified Person and adding the Latitude and Longitude to the ArrayList.
//		for(Person p:listOfPersons)
//		{
//			if(p.getCityOfBirth().toString().equalsIgnoreCase(cityToSearch) && p.getStateOfBirth().toString().equalsIgnoreCase(stateToSearch))
//			{
...
//	 * @return word A String of Person objects in a particular city. (String)
//	 */
//	public ArrayList<Person> searchForPeopleInCityObject(String cityToSearch,String stateToSearch)
//	{
//		Collections.sort(listOfPersons, new LastNameComparator());
//		
//		//Construction of ArrayList to hold Persons within the matching city and boolean to show a match was found.
//		ArrayList<Person> personList = new ArrayList<Person>();
//		boolean isIn = false;
//		
...
//	 * @param teamName Name of the Team in which the User is trying to find the specific roster.
...
//	 * <P>
//	 * Algorithm:<br>
//	 * 1. Compares component to firstName then lastName if needed to sort the ArrayList as wanted<br>
//	 * <P>
//	 * @param component :String which determines whether the sort will check by first/last name (String)
...
//	 * @return stateToCreate State that needs to be added to the list of States. (State)
//	 */
...
//	 * <P>
//	 * @param resultToPrint Result concluded by the User to be saved into the File.
//	 * @param fileOutput File in which to be written when saving any information.
//	 * @throws IOException Signals that an I/O exception has occurred. (Exception)
//	 */
//	public void writeFile(String resultToPrint, String fileOutput) throws IOException
//	{
//	
...
//	 * @throws IOException Signals that an I/O exception has occurred. (Exception)
//	 * @throws ClassNotFoundException Catch of Exception that file was not found or made.
//	 */
//	public String readFile(String fileOutput) throws IOException, ClassNotFoundException
//	{
...
//	 */
//	public boolean addPeople(String name,String dateOfBirth,State stateOfBirth,City cityOfBirth,String dateOfDeath)
//	{
...
//			{
//				
//				if(person.getName().equals(name) && person.getDateOfBirth().equals(dateOfBirth) && person.getStateOfBirth().getStateName().equals(stateOfBirth.getStateName()) && person.getCityOfBirth().getCityName().equals(cityOfBirth.getCityName()) && person.getDateOfDeath().equals(dateOfDeath))
//				{
//					return false;
//				}
//			}
//			else 
//			{
//				if(person.getName().equals(name) && person.getDateOfBirth().equals(dateOfBirth) && person.getStateOfBirth().getStateName().equals(stateOfBirth.getStateName()) && person.getCityOfBirth().getCityName().equals(cityOfBirth.getCityName()))
//				{
//					return false;
//				}
//			}
...
//		{
//			personToCreate = new Person(name,dateOfBirth,stateOfBirth,cityOfBirth,dateOfDeath,true);
//
//		}
//		else
//		{
//			personToCreate = new Person(name,dateOfBirth,stateOfBirth,cityOfBirth,true);
//		}
//
...
//		{
//			if(listOfPersons.get(i).getName().equals(oldPerson.getName()) && listOfPersons.get(i).getDateOfBirth().equals(oldPerson.getDateOfBirth()) && listOfPersons.get(i).getCityOfBirth().getCityName().equals(oldPerson.getCityOfBirth().getCityName()))
//			{
//				index = i;
//			}
//		}
...
//		for(City city:listOfJListCities)
//		{
//			if(city != null && city.getCityName().equals(cityToCheck) && city.getStateName().equals(state.getStateName()))
//			{
//				return false;
//			}	
//
...
//	 */
//	public boolean editPlace(City city,String cityName,String latitude,String longitude) 
//	{		
//		boolean shouldEdit = !cityName.equals(city.getCityName()) || !latitude.equals(city.getLatitude()) || !longitude.equals(city.getLongitude());
//		
//		return shouldEdit;
//	}
...
//	 * @return cityToCreate  City that needs to be added to the list of States. (City)
//	 */
//	public City cityIsInJList(String cityToCheck,State state,boolean shouldEdit)
//	{
//		
//		//Instantiates the City
//		City cityToCreate = null;
...
//	 * @return stateToCreate State that needs to be added to the list of States. (State)
//	 */
//	public Person personIsInJList(String name,String dateOfBirth,State stateOfBirth,City cityOfBirth,String dateOfDeath,boolean shouldEdit)
//	{
//		Person personToCreate = null;
//		
//		for(Person person:listOfJListPersons)
...
//			if(dateOfDeath != null && !dateOfDeath.equals(""))
//			{
//				personToCreate = new Person(name,dateOfBirth,stateOfBirth,cityOfBirth,dateOfDeath,shouldEdit);
//			}
//			else
//			{
//				personToCreate = new Person(name,dateOfBirth,stateOfBirth,cityOfBirth,shouldEdit);
//			}
//			listOfJListPersons.add(personToCreate);
//		}
//
...
//		{
//			Person m = personIsInJList(p.getName(),p.getDateOfBirth(),p.getStateOfBirth(),p.getCityOfBirth(),p.getDateOfDeath(),false);
