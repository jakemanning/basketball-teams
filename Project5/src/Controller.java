...
public class Controller 
{
	...
	/**
	 * When the User adds a person, he or she will then be added to the list of Persons.
	 */
	...
	/**
	 * When the User edits a person, he or she will then be changed and re-add in the list of Persons.
	 */
	...
	/**
	 * When the User deleted a person, he or she will then be deleted from the list of Persons.
	 */
	private class DeletePeopleListener implements ActionListener
	{
		...
		{			
			for(Person p:mainView.peopleSelected)
			{
				for(Integer year:DataBaseModel.getJListOfPersons().get(p.getTextName().toUpperCase()).getSeasonList().keySet())
				{
					DataBaseModel.getJListOfPersons().get(p.getTextName().toUpperCase()).getSeasonList().get(year).getRosterMap().remove(p.getTextName().toUpperCase());
				}
			...
			for(String id:DataBaseModel.getJListOfIdentifications().keySet())
			{
				...
				{
					mainView.getSeasonListModel().addElement(DataBaseModel.getJListOfIdentifications().get(id).getSeasonList().get(year));
				}
			}
		}
	}
	...
	/**
	 * When the User adds a place, that place will then be added to the list of Places.
	 */
	...
	/**
	 * When the User edits a place, that place will then be changed and re-added to the list of Places.
	 */
	...
	/**
	 * When the User deletes a place, that place will then be removed from the list of Places.
	 */
	private class DeletePlaceListener implements ActionListener
	{
		...
		{			
			...
			for(City c:mainView.placesSelected)
			{
				...
				ArrayList<Identification> identificationList = new ArrayList<Identification>(DataBaseModel.getJListOfIdentifications().values());
	...
	/**
	 * When the User adds a team, that team will then be added to the list of Teams under a specific season.
	 */
	...
	/**
	 * When the User edits a team, that team will then be edited and re-add to the list of Teams under a specific season.
	 */
	...
	/**
	 * When the User deletes a team, that team will then be removed from the list of Teams under a specific season.
	 */
	...
	/**
	 *When the User selects the Save option in the File menu, the current status of the lists is written to an external
	 *text file.
	 */
	private class SaveListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			int selectedValue = JOptionPane.showOptionDialog(null,"Choose one", "Input",JOptionPane.INFORMATION_MESSAGE,0, null,possibleValues, possibleValues[2]);
			if(selectedValue >= 0)
			{
				JFileChooser chooser = new JFileChooser(System.getProperty("user.dir"));
				FileNameExtensionFilter commaSeparatedFilter = new FileNameExtensionFilter("Comma Separated Values","csv");
				...
				if (returnValue == JFileChooser.APPROVE_OPTION)
				{
					...
					try
					{
						...
						if(possibleValues[selectedValue].equals("Places"))
						{
							...
							{
								bw.write(c.getCityName() + "; " + c.getStateName() + "; " + c.getLatitude() + "; " + c.getLongitude()); 
								bw.newLine();
							}
						...
						else if(possibleValues[selectedValue].equals("People"))
						{
							...
							for(Person p: DataBaseModel.getJListOfPersons().values())
							{
								if(p.getDateOfDeath() == null)
								{
									bw.write(p.getTextName() + "," + p.getDateOfBirth() +"," + p.getCityOfBirth().getCityName() + "," + p.getStateOfBirth()); 

								}
								else
								{

									bw.write(p.getTextName() + "," + p.getDateOfBirth() +"," + p.getCityOfBirth().getCityName() + "," + p.getStateOfBirth() + "," + p.getDateOfDeath());
								}
								bw.newLine();
						...
						else
						{
							...
							for(Identification id:DataBaseModel.getJListOfIdentifications().values())
							{
								...
								for(Integer year:id.getSeasonList().keySet())
								{
									...
									bw.write(season.getYear() + "; " + season.getTeamName() + "; " + season.getTeamCity().getCityName() + "; " + season.getTeamCity().getStateName() + "; " + season.getMembersForSaving());
									bw.newLine();
								}
							}
	...
	/**
	 * When the User selects the Load option in the File menu, a previous status of the lists is brought the Main Control.
	 */
	private class LoadListener implements ActionListener 
	{
		public void actionPerformed(ActionEvent e) 
		{
			if(!mainView.placesSaved)
			{
				...
				int selectedValue = JOptionPane.showOptionDialog(null,"Choose one", "Places Not Saved",JOptionPane.WARNING_MESSAGE, 2,null,possibleValues, possibleValues[2]);
				if(selectedValue >= 0)
				{
			...
			{
				String[] possibleValues = {"Discard", "Export","Save"};
				int selectedValue = JOptionPane.showOptionDialog(null,"Choose one", "People Not Saved",JOptionPane.WARNING_MESSAGE, 2,null,possibleValues, possibleValues[2]);
				if(selectedValue >= 0)
				{
					if(possibleValues[selectedValue].equals("Save"))
					{
			...
			{
				String[] possibleValues = {"Discard", "Export","Save"};
				int selectedValue = JOptionPane.showOptionDialog(null,"Choose one", "Teams Not Saved",JOptionPane.WARNING_MESSAGE, 2,null,possibleValues, possibleValues[2]);
				if(selectedValue >= 0)
				{
					if(possibleValues[selectedValue].equals("Save"))
					{
			...
			JOptionPane.showMessageDialog (null, "All data is saved. Continuing on.....", "Loading", JOptionPane.INFORMATION_MESSAGE);
			String[] possibleValues = {  "Teams","Players","Places"  };
			...
			if(selectedValue >= 0)
			{
				...
				else if(possibleValues[selectedValue].equals("Players") && !DataBaseModel.getJListOfCities().isEmpty())
				{
					JFileChooser chooser = new JFileChooser(System.getProperty("user.dir"));
					FileNameExtensionFilter commaSeparatedFilter = new FileNameExtensionFilter("Comma Separated Values","csv");
					chooser.addChoosableFileFilter(commaSeparatedFilter);
					...
					if (returnValue == JFileChooser.APPROVE_OPTION)
					{
						...
						try 
						{
							model.readDataFromFile(selectedFile.getName());
							mainView.degreeOfSeparation.setEnabled(true);
							mainView.jmiSave.setEnabled(true);
							mainView.jmiExport.setEnabled(true);
							mainView.jmiPieChart.setEnabled(true);
				...
				{
					System.out.println("You must pick Places if there are no places, and People if there are no people but there are Places.");
				}
	...
	/**
	 * When the User selects the Exit option in the File menu, the Container will close and the program will terminate.
	 */
	private class ImportListener implements ActionListener 
	{
		public void actionPerformed(ActionEvent e) 
		{
			...
			JOptionPane.showMessageDialog (null, "All data is saved. Continuing on....", "Importing", JOptionPane.INFORMATION_MESSAGE);
			JFileChooser chooser = new JFileChooser(System.getProperty("user.dir"));
			FileNameExtensionFilter binaryFilter = new FileNameExtensionFilter("Binary Files","bin");
			chooser.addChoosableFileFilter(binaryFilter);
			chooser.setFileFilter(binaryFilter);
	...
	private class TeamIDOKButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if(DataBaseModel.getJListOfIdentifications().containsKey(teamIdentificationView.getTeamID()))
			{
				...
				ArrayList<Season> seasonList = new ArrayList<Season>(DataBaseModel.getJListOfIdentifications().get(teamIdentificationView.teamIdentification.toUpperCase()).getSeasonList().values());
			...
			{
				System.out.println("Adding teamID " + teamIdentificationView.teamIdentification + ".....this may take up to 10 seconds");
				Identification newIdentification = new Identification(teamIdentificationView.teamIdentification);
				MapClass.putInMapListIfAbsent(DataBaseModel.getJListOfIdentifications(),newIdentification.getIdentification().toUpperCase(),newIdentification);
				addSeasonView = new AddSeasonView(model);
				setAddSeasonView(addSeasonView);
	...
	private class CityAddListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			state = MapClass.putInMapListIfAbsent(DataBaseModel.getJListOfStates(), state.getStateName().toUpperCase(), state);

			if(!DataBaseModel.getJListOfCities().containsKey(cityName.toUpperCase() + state.getStateName().toUpperCase()))
			{
				mainView.placesSaved = false;
				City city = new City(cityName, state,true);
				city = state.addToList(city);
				city = MapClass.putInMapListIfAbsent(DataBaseModel.getJListOfCities(), cityName.toUpperCase() + state.getStateName().toUpperCase(), city);
	...
	private class EditStateOKListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			if(!DataBaseModel.getJListOfCities().containsKey(editPlaceStateView.cityToEdit.toUpperCase() + stateSelected.toUpperCase()) && !DataBaseModel.getJListOfStates().get(editPlaceStateView.stateToEdit.toUpperCase()).getListOfCities().containsKey(editPlaceStateView.cityToEdit.toUpperCase() + stateSelected.toUpperCase()))
			{	

				DataBaseModel.getJListOfCities().remove(editPlaceStateView.cityToEdit.toUpperCase() + editPlaceStateView.stateToEdit.toUpperCase());
				City cityToReplace = DataBaseModel.getJListOfStates().get(editPlaceStateView.stateToEdit.toUpperCase()).getListOfCities().remove(editPlaceStateView.cityToEdit.toUpperCase() + editPlaceStateView.stateToEdit.toUpperCase());
				...
				DataBaseModel.getJListOfCities().put(editPlaceStateView.cityToEdit.toUpperCase() + stateSelected.toUpperCase(), cityToReplace);
				...
				editPlaceCityView.getCityNameField().setText(DataBaseModel.getJListOfCities().get(editPlaceStateView.cityToEdit.toUpperCase() + editPlaceStateView.stateToEdit).getCityName());
				editPlaceCityView.getCityLatitudeField().setText(DataBaseModel.getJListOfCities().get(editPlaceStateView.cityToEdit.toUpperCase() + editPlaceStateView.stateToEdit).getLatitude());
				editPlaceCityView.getCityLongitudeField().setText(DataBaseModel.getJListOfCities().get(editPlaceStateView.cityToEdit.toUpperCase() + editPlaceStateView.stateToEdit).getLongitude());
			}
			else
			{
	...
	private class EditCityOKListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			boolean shouldEdit = !DataBaseModel.getJListOfCities().containsKey(cityName.toUpperCase() + editPlaceStateView.stateToEdit) && !DataBaseModel.getJListOfStates().get(editPlaceStateView.stateToEdit.toUpperCase()).getListOfCities().containsKey(cityName.toUpperCase() + editPlaceStateView.stateToEdit.toUpperCase());
			...
			if(shouldEdit)
			{
				...
				DataBaseModel.getJListOfCities().put(editPlaceStateView.cityToEdit.toUpperCase() + editPlaceStateView.stateToEdit.toUpperCase(), cityToReplace);
	...
	private class PersonAddOKButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			//			
			//			boolean added = model.addPerson(name, dateOfBirth, stateOfBirth, cityOfBirth, dateOfDeath);
			//			personToAdd = model.personIsInJList(name, dateOfBirth, stateOfBirth, cityOfBirth,dateOfDeath,true);
			//
			//
	...
	private class PersonEditCityButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			editPersonCityView.setCityList(DataBaseModel.getJListOfStates().get(editPersonCityView.stateSelected.getStateName().toUpperCase()).getListOfCities().values().toArray(new City[0]));
		}
	}
		...
		public void valueChanged(ListSelectionEvent e) 
		{
			if(e.getValueIsAdjusting())
			{
				try
				{
					...
					{
						addSeasonView.peopleSelected = (ArrayList<Person>)addSeasonView.personList.getSelectedValuesList();
	...
	private class AddSeasonOKListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				mainView.teamsSaved = false;
				Season seasonToAdd = new Season(addSeasonView.yearSelected,addSeasonView.nameField.getText(),addSeasonView.citySelected,addSeasonView.personMap,true);
				seasonToAdd = MapClass.putInMapListIfAbsent(DataBaseModel.getJListOfIdentifications().get(teamIdentificationView.teamIdentification).getSeasonList(),seasonToAdd.getYear(),seasonToAdd);
				DataBaseModel.getJListOfIdentifications().get(teamIdentificationView.teamIdentification).sortSeasonList();
	...
	/**
	 * Sets the Model of the program that will or is being affiliate with the Controller.
	 * <P>
	 * @param model The model affiliated with this Controller.
	 */
	...
	public void setAddPersonView(AddPersonView addPersonView)
	{
		...
		//		this.addPersonView.getCancelButton().addActionListener(new CancelListener());
	}
	...
	public void setAddPlaceStateView(AddPlaceStateView addPlaceStateView)
	{
		...
		// TODO work on cancel button
		//		this.addPlaceStateView.getCancelButton().addActionListener(new CancelListener());
	...
	public void setAddPlaceCityView(AddPlaceCityView addPlaceCityView)
	{
		...
		// TODO work on cancel button
		//		this.addPlaceCityView.getCancelButton().addActionListener(new CancelListener());
	...
	public void setSorryView(SorryView sorryView)
	{
		...
		// TODO work on cancel button
		//		this.sorryView.getCancelButton().addActionListener(new CancelListener());
	}
	...
	public void setAddPersonStateView(AddPersonStateView addPersonStateView)
	{
		...
		// TODO work on cancel button
		//		this.addPersonStateView.getCancelButton().addActionListener(new CancelListener());
	...
	public void setAddPersonCityView(AddPersonCityView addPersonCityView)
	{
		...
		// TODO work on cancel button
		//		this.addPersonCityView.getCancelButton().addActionListener(new CancelListener());
	}
	...
	public void setEditPersonStateView(EditPersonStateView editPersonStateView)
	{
		...
		// TODO work on cancel button
		//		this.editPersonStateView.getCancelButton().addActionListener(new CancelListener());
	...
	public void setEditPersonCityView(EditPersonCityView editPersonCityView)
	{
		...
		// TODO work on cancel button
		//		this.editPersonCityView.getCancelButton().addActionListener(new CancelListener());
